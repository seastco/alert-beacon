...
class TestAlertManager(unittest.TestCase):
    ...
    def test_process_alerts(self, mock_create_alert, MockNotificationService, MockStorageService):
        ...
        mock_storage_service.get_subscribers.return_value = ['+1234567890', '+0987654321']
        ...
        mock_alert.format_alert.side_effect = lambda item: f"ALERT! {item['properties']['mag']} magnitude earthquake detected {item['properties']['place']} at {item['properties']['time']}"
        mock_create_alert.return_value = mock_alert

        # Initialize AlertManager with mocks
        alert_manager = AlertManager(notification_service=mock_notification_service, storage_service=mock_storage_service)
        ...
        mock_notification_service.send_alert.assert_called_with(expected_message, ['+1234567890', '+0987654321'])
